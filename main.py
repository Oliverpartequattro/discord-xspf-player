...
async def on_ready():
    ...
    message = """
    **Initializing the following commands: <a:loading:1153202857969983488>**
    ...
    ✔️ `!play_all` - Accepted argument(s): `None` **(Plays the entire playlist)**
...
async def play(ctx, id):
    ...
    if ctx.voice_client == None:
        await ctx.send("I'm not in a voice channel yet. Use `!join` to add me in a voice channel!")
        return None 
...
`Duration`  : {duration}"""
    ...
    # Path to your FFmpeg executable (modify this if necessary)
    ffmpeg_path = r"D:\Program Files\ffmpeg\bin\ffmpeg.exe"  # Modify this path if necessary
    
    # Play the song using FFmpeg
    audio_source = discord.FFmpegPCMAudio(mainlist[int(id) - 1][1], executable=ffmpeg_path)
...
async def play_all(ctx):
    ...
    if ctx.voice_client.is_playing():
        ctx.voice_client.stop()  # Stop the current playback before starting a new song

    # Start playing the first song
    await play_next_song(ctx, 0)  # Start with the first song in the playlist


async def play_next_song(ctx, index):
    """Play the song at the given index and move to the next one after it finishes."""
    if index >= len(mainlist):
        await ctx.send("Finished playing the playlist!")
        return

    song = mainlist[index]
    imgpath = (urllib.parse.unquote(song[5])).replace("file:///",'')
    seconds = math.floor((int(song[4])/1000))
    duration = str(datetime.timedelta(seconds=seconds))
    details = f"""`Artist`    : {song[2]}
`Album`     : {song[3]}
`Duration`  : {duration}"""
    
    # Create embed
    file = discord.File(imgpath, filename='art.png')
    embed = discord.Embed(title=f"{song[0]}", description = details, color=discord.Color.from_rgb(255,187,187), timestamp=datetime.datetime.now(datetime.timezone.utc))
    embed.set_thumbnail(url='attachment://art.png')
    embed.set_author(name="Currently Playing")
    embed.set_footer(text=f"Requested by {ctx.author}")
    await ctx.send(file=file, embed=embed)
    ...
    # Play the song using FFmpeg
    audio_source = discord.FFmpegPCMAudio(song[1], executable=ffmpeg_path)

    # Set up the after callback to play the next song after the current one finishes
    def after_playing(error):
        if error:
            print(f"Error occurred while playing: {error}")
        # Play the next song in the playlist after this one finishes
        asyncio.run_coroutine_threadsafe(play_next_song(ctx, index + 1), bot.loop)
...
async def info(ctx):
  ...
  ✦ `!play_all` - Accepted argument(s): `None` **(Plays the entire playlist)**
